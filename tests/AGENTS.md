...
- **Migration** tests in `migration.rs` cover the path somebody arrives on from git-crypt: sealing files git already tracks, and the first commit after that. Every other suite seals files git never had, which is why this one exists.
- **Deploy** tests in `deploy.rs` cover `unseal`: opening a vault with no repository and no git, with a key file or an SSH identity. They carry a real ed25519 key pair as a constant rather than shelling out to `ssh-keygen`, so they stay hermetic.
...
- **`fs::canonicalize` returns `\\?\C:\…` on Windows**, and git refuses such a path with `fatal: unknown error occurred while reading the configuration files`. The harness and `src/paths.rs` strip that prefix; anything that resolves a path and hands it to git must do the same.
- **Windows has no FIFO and no symlink an unprivileged process can create**, so `a_secret_that_is_neither_a_file_nor_a_link_is_named` and `a_symlink_does_not_hide_where_the_binary_really_lives` are `#[cfg(unix)]`. Git for Windows ships an MSYS `mkfifo` that exits zero and leaves an ordinary file behind, so its exit status alone does not say the fixture exists.
- **`age` writes CRLF armor on Windows**, so `keys::wrap` normalises the envelope and `an_envelope_has_the_same_bytes_on_every_platform` pins it. Without that, `.vault/keys` would depend on the platform that wrote it, every access change would show as a whole-file diff, and `line_ending_translation_cannot_corrupt_the_vault` could not tell that apart from git rewriting the file.
...
- **Untracking is one `git update-index` call, fed through stdin.** Taking the paths one at a time refreshes the index in between, which runs the plaintext filter over the paths not yet removed, and that filter's whole job is to refuse. Doing it per path leaves the repository in a state where no git command works at all. `add_repairs_declared_secrets_that_are_still_tracked` needs two leaked files to catch this; with one it passes either way.
- **`git vault add` untracks before it declares.** While nothing is declared the filter is inert, so the removal cannot trip over it. That ordering is correct by construction; the batching above relies on git's internal refresh behaviour, which is why both are there and why this suite runs against five git versions.
- **"Is this secret leaking?" is a question about the index, never about the worktree or a diff.** `git diff --cached` lists deletions, so the staged removal that `git vault add` just achieved reads as a leak and refuses the very first commit. Enumerating the worktree misses a tracked plaintext secret that is no longer on disk, which made `doctor` report `ok` over a real leak. Both use `Repo::tracked_secrets`.
...
- **Labels come from `GIT_VAULT_LABEL`, then `GIT_AUTHOR_EMAIL`**, so output that mentions a recipient stays the same on every machine. The harness sets the address. The harness identity's label is `test@example.com`, so a test about somebody else's label has to look at that recipient's line rather than at the whole listing.
- **An SSH key carries its own comment, and that comment is not a label.** `.vault/recipients` is whitespace-delimited, so a key stored with its comment leaves the label in a fourth field that nothing reads, which silently replaced the label and made `revoke <label>` fail. `Recipient::new` drops the comment on the way in; `an_ssh_key_keeps_the_label_you_gave_rather_than_its_own_comment` pins the round trip.
- **An SSH private key does not carry its public key**, and `age` does not expose one, so `Identity::public` reads the sibling `.pub` file and returns `None` when it is absent. A test that shares an SSH key has to write both files.
